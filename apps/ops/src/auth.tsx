...
/**
 * 运营角色（P5）：operator / super_admin 可进入运营端；auditor（全局只读审计）
 * 与普通用户拒绝。平台角色单源为 DB user_role，dev token 传入角色仅作开发兜底。
 */
export const OPS_ROLES = ['operator', 'super_admin'] as const;

/** 运营端全部页面可写（运营域不含只读审计语义）；保留 usePerm 形态与 admin 同构。 */
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const loginDev = async (username: string, roles?: string[]) => {
    const resp = await fetch('/api/v1/auth/dev/token', {
      ...
      // 运营端 dev 登录默认挂 operator（运营角色；平台角色单源为 DB user_role）
      body: JSON.stringify({ username, roles: roles ?? ['operator'] }),
    });
    if (!resp.ok) {
